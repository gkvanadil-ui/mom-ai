...
# 1. 페이지 설정 (가로로 넓게 배치하여 칸이 깨지지 않게 합니다)
...
# ⭐ 따님, 여기에 구글 시트 링크를 꼭 넣어주세요! (현재 따님이 주신 링크입니다)
...
with row1_col2:
    ...
    st.session_state.m_tar = st.text_input("🎁 추천 선물 대상", value=st.session_state.m_tar, placeholder="예: 친구 생일, 나를 위한 선물")
...
def process_ai(guide):
    ...
    # 입력된 모든 정보를 하나로 합쳐서 AI에게 전달
    info = f"작품명:{st.session_state.m_name}, 소재:{st.session_state.m_mat}, 기간:{st.session_state.m_per}, 대상:{st.session_state.m_tar}, 상세내용:{st.session_state.m_det}"
...
with tabs[0]: # 판매글 쓰기
    ...
    for k in ["인스타", "아이디어스", "스토어"]:
        if st.session_state.texts.get(k):
            ...
            st.text_area(f"{k} 내용", value=st.session_state.texts[k], height=250, key=f"text_output_{k}")
...
with tabs[3]: # 창고 (구글 시트 연동)
    ...
    try:
        # SHEET_URL을 직접 사용하여 에러를 해결합니다.
        df = conn.read(spreadsheet=SHEET_URL, ttl=0)
        
        if st.button("✨ 지금 입력한 정보를 창고에 저장하기"):
        ...
        for i, r in df.iterrows():
            with st.expander(f"📦 {r['name']}"):
                ...
                if st.button("📥 이 정보 다시 불러오기", key=f"gs_load_{i}"):
                    # 불러온 정보를 입력 칸에 쏙 넣어줍니다.
