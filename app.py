...
with col2:
    st.session_state.m_per = st.text_input("⏳ 제작 소요 기간", value=st.session_state.m_per, placeholder="예: 주문 후 3일 이내")
    st.session_state.m_tar = st.text_input("🎁 추천 선물 대상", value=st.session_state.m_tar, placeholder="예: 친구 생일, 나를 위한 선물")

# 상세 설명은 넓게 한 줄 사용
st.session_state.m_det = st.text_area("✨ 정성 포인트와 상세 설명", value=st.session_state.m_det, height=180, placeholder="작가님의 정성이 들어간 이야기를 자유롭게 적어주세요.")
...
with tabs[3]: # 창고
    ...
    try:
        ...
        st.warning("구글 시트 링크 설정을 확인해 주세요! 🌸")
